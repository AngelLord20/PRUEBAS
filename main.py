...
class SistemaCompras:
    ...
    def crear_productos_ejemplo(self):
        ...
        productos_ejemplo = [
            {'id': 1, 'nombre': 'Laptop HP', 'precio': 899.99, 'descripcion': 'Laptop HP 15 pulgadas', 'stock': 10},
            {'id': 2, 'nombre': 'Mouse Inalámbrico', 'precio': 25.50, 'descripcion': 'Mouse inalámbrico Logitech', 'stock': 50},
            {'id': 3, 'nombre': 'Teclado Mecánico', 'precio': 89.99, 'descripcion': 'Teclado mecánico RGB', 'stock': 20},
            {'id': 4, 'nombre': 'Monitor 24"', 'precio': 199.99, 'descripcion': 'Monitor LED 24 pulgadas', 'stock': 15},
            {'id': 5, 'nombre': 'Auriculares', 'precio': 45.00, 'descripcion': 'Auriculares con micrófono', 'stock': 30}
        ]
        
        with open('productos.csv', 'w', newline='', encoding='utf-8') as file:
            fieldnames = ['id', 'nombre', 'precio', 'descripcion', 'stock']
    ...
    def agregar_al_carrito(self):
        ...
        if producto:
            # Agregar al carrito
            self.carrito.agregar_producto(producto, 1)
    ...
    def modificar_cantidad(self):
        ...
        entry_cantidad.insert(0, str(self.carrito.items[index].cantidad))
...
Stock disponible: {producto.stock}
    ...
    def actualizar_visualizaciones(self):
        ...
        for producto in self.lista_productos:
            self.text_lista.insert(tk.END, f"{producto.id}. {producto.nombre} - ${producto.precio:.2f}\n")
        
        # Pila de órdenes
        self.text_pila.delete(1.0, tk.END)
        for orden in reversed(self.pila_ordenes):
            self.text_pila.insert(tk.END, f"Orden #{orden.id} - {orden.fecha} - ${orden.total:.2f}\n")
        
        # Cola de pagos
        self.text_cola.delete(1.0, tk.END)
        for pago in self.cola_pagos:
    ...
    def guardar_orden_csv(self, orden):
        ...
        try:
            with open('ordenes.csv', 'a', newline='', encoding='utf-8') as file:
